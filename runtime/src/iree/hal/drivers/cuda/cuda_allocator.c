...
static void iree_hal_cuda_allocator_deallocate_buffer(
  ...
  //
  // WARNING: we may hit this code path from buffers allocated from
  // iree_hal_cuda_device_queue_alloca as users aren't required to deallocate
  // them with iree_hal_cuda_device_queue_dealloca. It's invalid to double-free
  // so we ignore the free here and let the pooling allocator handle it.
